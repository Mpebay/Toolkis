...
const CategoryPage = () => {
    ...
    const {main,param} = useParams()
    const dispatch = useDispatch()
    useEffect(() => {
        const filtered = allItems.filter(item => item?.subcategory.toLowerCase() === param)
      setItems(filtered)
    }, [allItems,param])
  ...
  return (
    <div className='bg-[#f0ebe3]'>
        <p className='font-bold text-[#053b50] pl-4 text-2xl'><Link to={"/"}>/home</Link>/<Link to={`/category/${main.toLocaleLowerCase()}`}>{main}</Link>/{param}</p>
